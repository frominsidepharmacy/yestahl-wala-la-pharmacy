...
`business_preview.yml` releases the oldest QC-passed carousel across its isolated `day` and `evening` inventory lanes once daily at 20:00 Dubai time. Released items are atomically changed from `ready.json` to `previewed.json`, so a topic is never selected twice. Business publication keys start with `biz-`; `publish.yml` routes only those keys to the dedicated `BUSINESS_META_ACCESS_TOKEN` and `BUSINESS_INSTAGRAM_USER_ID`. Every other key continues through the pharmacy job for `@dramrouaboubakr`, with separate artifacts, credentials, and approval state.
